"""Country list for the "default country code" dropdown.

Region codes come from phonenumbers (so every entry is one the normaliser
accepts); display names are kept here because phonenumbers ships none.
"""

import phonenumbers

# Shown at the top of the dropdown, in this order.
POPULAR = ["US", "GB", "BD", "IN", "PK", "AE", "SA", "MY", "ID", "NG"]
...
NAMES = {
    ...
    "CC": "Cocos (Keeling) Islands", "CD": "Congo (DRC)",
    "CF": "Central African Republic", "CG": "Congo (Republic)",
...
def region_choices():
    """Return (popular, others): lists of {code, name, dial} dicts.

    ``popular`` keeps the POPULAR order; ``others`` holds every remaining
    region phonenumbers supports, sorted by name.
    """
    supported = set(phonenumbers.SUPPORTED_REGIONS)
    popular = [_entry(c) for c in POPULAR if c in supported]
    rest = sorted(
        (_entry(c) for c in supported if c not in POPULAR),
        key=lambda e: e["name"],
    )
    return popular, rest
